...
func mustInsertEntities(t *testing.T, e engine.Engine, entities []any) {
	...
	for _, entity := range entities {
		...
		case *internal.MessageSubscriptionEntity:
			err = pgCtx.MessageSubscriptions().Insert(entity)
		...
		case *internal.ProcessEntity:
			_, err = pgCtx.Processes().Insert(entity)
		...
		case *internal.SignalSubscriptionEntity:
			err = pgCtx.SignalSubscriptions().Insert(entity)
		...
		case *internal.UserTaskEntity:
			err = pgCtx.UserTasks().Insert(entity)
