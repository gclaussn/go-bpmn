...
func TestContinueExecution(t *testing.T) {
	...
	t.Run("sub-process", func(t *testing.T) {
		// given
		graph := mustCreateGraph(t, "sub-process/start-end.bpmn", "startEndTest")
		...
		t.Run("scope SUSPENDED", func(t *testing.T) {
			...
			assert.Equal("subProcessStartEvent", executions1[0].BpmnElementId)
			assert.Equal(execution, executions1[0].parent)
		})
	})
}
