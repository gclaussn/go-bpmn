...
func TestExecute(t *testing.T) {
	...
	t.Run("list-conf", func(t *testing.T) {
		...
		assert.Contains(out, "GO_BPMN_HTTP_BASIC_AUTH_USERNAME=")
		assert.Contains(out, "GO_BPMN_HTTP_BASIC_AUTH_PASSWORD=")
		assert.Contains(out, "GO_BPMN_HTTP_BIND_ADDRESS=127.0.0.1:8080")
		assert.Nil(err)
	})

	t.Run("list-conf with env", func(t *testing.T) {
		out, err := execute("list-conf", "--env", "GO_BPMN_ENGINE_ID=engine-id", "-e", "GO_BPMN_HTTP_BASIC_AUTH_PASSWORD=password")
		assert.Contains(out, "GO_BPMN_ENGINE_ID=engine-id", "should override default value")
		assert.Contains(out, "GO_BPMN_HTTP_BASIC_AUTH_PASSWORD=password", "should set value")
		assert.Nil(err)
	})
	...
	t.Run("list-conf with env-file", func(t *testing.T) {
		...
		f.WriteString("GO_BPMN_HTTP_BASIC_AUTH_PASSWORD=password\n")
		f.WriteString("\n") // empty lines should be ignored

		out, err := execute("list-conf", "--env-file", f.Name())
		assert.Contains(out, "GO_BPMN_ENGINE_ID=engine-id", "should override default value")
		assert.Contains(out, "GO_BPMN_HTTP_BASIC_AUTH_PASSWORD=password")
		assert.Nil(err)
	})
